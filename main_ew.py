...
def port_opt(period_list, industry_list, barra_list, past_weight, stock_today, index_today, barra_limit, turnover=0.30, num_stock=1000):
    ...
    # use equal weight for the top num_stock stocks
    stock_today = stock_today.sort_values('pred', ascending=False)
    stock_today = stock_today.head(num_stock)
    stock_today['weight'] = 1/num_stock

    weights_value = stock_today[['Symbol','weight']].reset_index(drop=True)
