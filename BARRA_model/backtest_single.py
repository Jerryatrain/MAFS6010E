...
# TODO: simple version optimization, no constraints
...
def backtest_single(score_df, turnover_punish, file_name):
    ...
    rolling_window = 20
    std_num = 1
    index_return['vix'] = index_return['close'].rolling(window=20).std()  # 指数波动率
    index_return['vix'] = index_return['vix'].shift(1)  # 用昨日波动率作为参考

    index_return['boll_up'] = index_return['vix'].rolling(window=rolling_window).mean() + std_num * index_return['vix'].rolling(
        window=rolling_window).std()
    index_return['boll_down'] = index_return['vix'].rolling(window=rolling_window).mean() - std_num * index_return['vix'].rolling(
        window=rolling_window).std()
    
    index_return['barra_std_num'] = 0.3
    index_return.loc[index_return['vix'] > index_return['boll_up'], 'barra_std_num'] = 0.1
    index_return.loc[index_return['vix'] < index_return['boll_down'], 'barra_std_num'] = 0.5

    df_index['trade_date'] = df_index.index

    df_index = pd.merge(df_index, index_return[['trade_date', 'barra_std_num']], on='trade_date', how='left').set_index('trade_date')
    ...
    for barra_ in barra_list:
        ...
        df_index[barra_ + '_lower'] = df_index[barra_] - df_index['barra_std_num'] * df_index[barra_ + '_std']
        df_index[barra_ + '_upper'] = df_index[barra_] + df_index['barra_std_num'] * df_index[barra_ + '_std']
    ...
    # for date_num in range(0, 598):
            ...
            try:
                ...
                try:
                    ...
                    for barra_ in barra_list:
                        ...
                        #     (df_index[barra_] + barra_std_num * df_index[barra_ + '_std']).loc[date_]
                        df_index.loc[date_, barra_ + '_lower'] = \
                            (df_index[barra_] - df_index['barra_std_num']/2 * df_index[barra_ + '_std']).loc[date_]
                        df_index.loc[date_, barra_ + '_upper'] = \
                            (df_index[barra_] + df_index['barra_std_num']/2 * df_index[barra_ + '_std']).loc[date_]
        ...
        diff = (today_barra['barra_beta'].sum() - index_next['barra_beta']) / (
                    (index_next['barra_beta_upper'] - index_next['barra_beta']) / index_next['barra_std_num'])
        ...
        diff = (today_barra['barra_earnyild'].sum() - index_next['barra_earnyild']) / (
                (index_next['barra_earnyild_upper'] - index_next['barra_earnyild']) / index_next['barra_std_num'])
        ...
        diff = (today_barra['barra_resvol'].sum() - index_next['barra_resvol']) / (
                (index_next['barra_resvol_upper'] - index_next['barra_resvol']) / index_next['barra_std_num'])
        ...
        diff = (today_barra['barra_growth'].sum() - index_next['barra_growth']) / (
                (index_next['barra_growth_upper'] - index_next['barra_growth']) / index_next['barra_std_num'])
        ...
        diff = (today_barra['barra_btop'].sum() - index_next['barra_btop']) / (
                (index_next['barra_btop_upper'] - index_next['barra_btop']) / index_next['barra_std_num'])
        ...
        diff = (today_barra['barra_leverage'].sum() - index_next['barra_leverage']) / (
                (index_next['barra_leverage_upper'] - index_next['barra_leverage']) / index_next['barra_std_num'])
        ...
        diff = (today_barra['barra_liquidty'].sum() - index_next['barra_liquidty']) / (
                (index_next['barra_liquidty_upper'] - index_next['barra_liquidty']) / index_next['barra_std_num'])
